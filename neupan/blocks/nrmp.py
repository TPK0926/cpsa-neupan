...
class NRMP(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.risk_margin = value_to_tensor(0.0)
        self._per_point_margins = None  # numpy array (max_num,) or None

        self.adjust_parameters = (
            [self.q_s, self.p_u]
            if self.no_obs
            else [self.q_s, self.p_u, self.eta, self.d_max, self.d_min, self.risk_margin]
        )
    ...
    def set_risk_margin(self, q_hat: float):
        """Set the conformal risk margin (q_hat) for distance constraint tightening.

        After calling this, the constraint becomes d >= d_min + q_hat.
        """
        self.risk_margin = value_to_tensor(q_hat)
        self._per_point_margins = None  # clear per-point when using global
        self.adjust_parameters = (
            [self.q_s, self.p_u]
            if self.no_obs
            else [self.q_s, self.p_u, self.eta, self.d_max, self.d_min, self.risk_margin]
    ...
    def set_per_point_margins(self, margins: np.ndarray):
        """Set per-point risk margins (e.g., from distance-weighted budget allocation).

        Args:
            margins: 1D numpy array of shape (n_points,). Each element is the
                     additional margin for that obstacle point's constraint.
        """
        self._per_point_margins = margins
        # When using per-point margins, set global risk_margin to 0
        self.risk_margin = value_to_tensor(0.0)
        self.adjust_parameters = (
            [self.q_s, self.p_u]
            if self.no_obs
    ...
    def generate_coefficient_parameter_value(self, mu_list, lam_list, point_list):
        ...
        if self.no_obs:
            ...
            # Apply per-point risk margins to fb_list
            # Adding margin to fb[i] tightens: fa[i]@xy - (fb[i]+margin) >= indep_dis
            if self._per_point_margins is not None:
                for t in range(self.T):
                    mu = mu_list[t + 1]
                    pn = min(mu.shape[1], self.max_num)
                    n = min(len(self._per_point_margins), pn)
                    if n > 0:
                        margin_t = to_device(torch.tensor(
                            self._per_point_margins[:n].reshape(-1, 1),
                            dtype=torch.float32
                        ))
                        fb_list[t][:n, :] = fb_list[t][:n, :] + margin_t
    ...
    def adjust_parameter_define(self, **kwargs):
        ...
        risk_margin: additional margin from conformal risk calibration (default 0 = disabled)
        """
        ...
        self.para_risk_margin = cp.Parameter(
            name="para_risk_margin", value=kwargs.get("risk_margin", 0.0), nonneg=True
        )
    ...
    def bound_dis_constraints(self):
        ...
        constraints += [cp.min(self.indep_dis) >= self.para_d_min + self.para_risk_margin]
